...
- `HomePage` — 首页
- `AboutPage` — 关于页（生活/工作双视图，生活页采用图片、观点与兴趣信息双栏布局）
...
| home | `sage` | 青瓷米白 |
| about | `sage` | 青瓷米白 |
| projects | `github` | GitHub 黑白灰 |
| notes | `ocean` | 深蓝黑 |
| contact | `black` | 黑 |

主题映射定义在 [src/App.tsx](src/App.tsx)，主题色值定义在 [src/themes.ts](src/themes.ts)，通过 inline `style` 传递。
