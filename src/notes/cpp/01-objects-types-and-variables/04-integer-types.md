...
先用一个只保留 `0～7` 的数值模型说明规则：数学结果为 `9` 时，减去整个范围的大小 `8`，对应到 `1`；结果为 `-1` 时，加上 `8`，对应到 `7`。越界时可以反复加上或减去 `8`，直到结果回到这个范围。

这种计算称为**模运算（modular arithmetic）**，这里的 `8` 称为**模（modulus）**。两个整数之差是模的整数倍时，称它们对该模**同余（congruent）**；例如 `9` 与 `1` 模 `8` 同余，`-1` 与 `7` 也模 `8` 同余。

C++ 的无符号算术以该类型可表示值的数量为模。若无符号整数类型的位宽为 $N$（这里指用于表示值的位数），其范围是 $0$ 到 $2^N-1$，共有 $2^N$ 个值，因此以 $2^N$ 为模。运算的数学结果超出范围时，按上述方式得到范围内与它同余的值。

**无符号算术的模规则不能套用到有符号算术上。**有符号整数运算的数学结果超出结果类型范围时会产生未定义行为，C++ 不保证它回绕到另一端。
