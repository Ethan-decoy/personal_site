...
初始化在对象创建时建立它的初始状态，赋值（assignment）则修改一个已经存在的对象所保存的值。
...
第一行创建并初始化 `remaining`；第二行中的 `=` 是赋值运算符。左操作数 `remaining` 指代需要修改的对象，右操作数 `9` 是提供新值的表达式。整个 `remaining = 9` 构成一个赋值表达式（assignment expression）。

赋值表达式求值后，`remaining` 保存的值由 `12` 替换为 `9`。这个过程不会创建新的对象，也不会改变原有对象的类型；`remaining` 仍然是同一个 `int` 对象。相关规则可参阅 [C++ 工作草案中的赋值与复合赋值运算符](https://eel.is/c%2B%2Bdraft/expr.assign)。
...
赋值、复合赋值、自增和自减都会改变对象的状态，这种在产生表达式结果之外发生的变化称为副作用（side effect）。它们作为独立语句出现时，修改通常一目了然；嵌入更大的表达式后，读者还需要同时追踪子表达式的结果和对象状态的变化。
