...
此前接触的声明语句和表达式语句，都可以各自构成一条语句。C++ 还允许将零条或多条语句放在一对花括号之间，使它们在语法上组成一个整体：
...
花括号内部包含一条声明语句和一条表达式语句，而包括花括号在内的整个结构本身也是一条语句。**这种能够容纳多条语句的结构称为复合语句（compound statement），也称为代码块（block）。**
