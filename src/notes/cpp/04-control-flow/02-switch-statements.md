...
执行到 `switch` 时，控制表达式 `command` 只求值一次。`char` 属于整数类型，并在匹配前进行相应的整数提升。

当前值与 `case 'p':` 匹配，程序从这个标签后面的语句开始执行，把 `speed_limit_kmh` 改为 `40`。随后执行的 `break` 语句结束当前 `switch`，使控制转移到整条语句之后，因此速度限制最终保持为 `40`。
...
**彼此独立的入口必须保证相应执行路径在到达下一个标签前离开 `switch`。**
