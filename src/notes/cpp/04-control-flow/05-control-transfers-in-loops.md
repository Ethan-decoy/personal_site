...
按源码中的控制关系推演，`value` 到达 `2` 时，`continue` 会绕过 `++value`，使条件依赖的状态无法继续变化。

本例也没有其他终止路径或标准认可的进展操作，因此不满足[前向进展要求](03-condition-controlled-loops.md#终止条件与状态变化)。它在 C++23 中具有未定义行为，不能据此保证实际程序始终停留在 `value == 2` 的循环中。
