...
这段定义只会再次发起相同调用，没有让内层调用返回的分支。它与[缺少前向进展的循环](../04-control-flow/03-condition-controlled-loops.md#终止条件与状态变化)一样，既没有终止路径，也没有标准认可的进展操作。在 C++23 中，调用这个函数会产生未定义行为，不能依赖它一直增加调用层数或以某种固定方式失败。

有终止条件的递归仍可能超过实现可用资源。语言没有规定可移植的最大递归层数，也没有规定资源耗尽时统一产生某个可捕获异常。**可靠的递归不仅需要写出终止条件，还需要保证输入沿实际执行路径持续接近这个条件。**

本篇描述的是 C++ 语义上的嵌套调用关系。实现通常怎样保存尚未完成的调用，可继续参阅附章[调用栈与栈帧](deep-dives/01-call-stack-and-stack-frames.md#栈帧承载一次活动调用的实现状态)。
