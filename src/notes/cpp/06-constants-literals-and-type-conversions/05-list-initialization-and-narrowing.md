...
使用花括号进行列表初始化（list-initialization）时，若初始化需要窄化转换，程序在语言上就不合法：
...
第三行没有使用列表初始化。语言允许把 `double` 值转换为 `int`，丢弃小数部分，再初始化 `copy_count`。编译器可能对此发出警告，但这里的复制初始化在语言上是合法的。
...
以下示例假设 `short` 的值域小于 `int`；在这一前提下，常量表达式例外决定初始化是否成立：
