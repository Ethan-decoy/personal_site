...
本篇只修改既有 `double` 元素的值，这不会改变元素身份。容器能够改变元素数量，但保存的元素引用还必须遵守[序列存储与引用有效性](04-sequence-storage-and-reference-validity.md#追加元素怎样影响引用)中的条件；容器仍然存在，并不保证它曾经管理的每个元素都仍然有效。
