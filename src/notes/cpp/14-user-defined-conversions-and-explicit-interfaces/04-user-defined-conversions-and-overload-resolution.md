...
下面先用 `class pressure_pa;` 声明类名，完整定义写在下方；此处只声明接收 `const pressure_pa&` 的构造函数，还不需要知道 `pressure_pa` 的成员与大小：
...
一条用户定义转换序列也不能任意串联转换。[用户定义转换序列与重载排序](deep-dives/01-user-defined-conversion-sequences-and-overload-ranking.md#一条序列固定分成三段)进一步拆解它的组成、数量限制和内部排序边界。
