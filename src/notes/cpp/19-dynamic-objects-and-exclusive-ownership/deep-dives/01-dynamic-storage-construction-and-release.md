...
本篇讨论普通单个类对象，使用标准库默认提供的全局分配与释放方式，类不定制这些操作；构造函数直接初始化成员，析构不向外抛出异常。示例中的手工创建和释放用于观察机制，日常独占管理仍采用 [make_unique 与 unique_ptr](../01-creating-and-owning-dynamic-objects.md)。
...
1. 按[构造失败的规则](../../18-exception-propagation-and-failure-state/03-construction-failure-and-member-cleanup.md)清理已经完成构造的成员。本例只有 `double` 成员，没有用户编写的成员析构动作；若成员是已经建成的独占拥有者，则它负责清理自己的目标。
2. 归还本次为传感器取得的存储。
3. 异常继续传播，进入匹配的处理者。
