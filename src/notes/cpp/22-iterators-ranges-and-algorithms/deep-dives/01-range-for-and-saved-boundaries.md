...
只需读取或修改已有读数时，范围 for 能直接表达任务。若还要追加新数据，可以先完成本次遍历，再追加，并在下一次遍历时重新取得边界。需要一边查找一边删除时，则使用[删除操作返回的位置](../04-container-mutation-and-iterator-validity.md)继续推进；该返回值描述修改后的序列，不能让范围 for 继续沿用原先保存的位置。
