...
export default function AboutPage({ theme, onNavigate, aboutView }: { theme: Theme; onNavigate: (s: Section, sub?: AboutView) => void; aboutView?: AboutView }) {
  ...
  const values = [
    ...
    { title: t('about.values.resilience'), desc: t('about.values.resilience.d') },
    { title: t('about.values.excellence'), desc: t('about.values.excellence.d') },
    { title: t('about.values.learning'), desc: t('about.values.learning.d') },
    { title: t('about.values.independence'), desc: t('about.values.independence.d') },
  ]
  ...
  const mbtiDims = [
    { key: 'I', label: t('mbti.I'), pct: 86, left: t('mbti.I.left'), right: t('mbti.I.right'), color: '#5A7A82', desc: t('mbti.I.d') },
    { key: 'N', label: t('mbti.N'), pct: 58, left: t('mbti.N.left'), right: t('mbti.N.right'), color: '#B8944F', desc: t('mbti.N.d') },
    { key: 'T', label: t('mbti.T'), pct: 85, left: t('mbti.T.left'), right: t('mbti.T.right'), color: '#5E8268', desc: t('mbti.T.d') },
    { key: 'J', label: t('mbti.J'), pct: 71, left: t('mbti.J.left'), right: t('mbti.J.right'), color: '#7B6B8B', desc: t('mbti.J.d') },
    { key: 'T2', label: t('mbti.T2'), pct: 51, left: t('mbti.T2.left'), right: t('mbti.T2.right'), color: '#A06060', desc: t('mbti.T2.d') },
  ]
  ...
  const skills = [
    { label: t('about.skills.algo'), items: [t('about.skills.algo.0'), t('about.skills.algo.1'), 'OpenCV', 'Eigen'] },
        ...
        <div key="personal" style={{ animation: 'fade-up 0.6s ease-out both', animationDelay: '150ms' }}>
          <div className="flex flex-col md:flex-row gap-8 md:gap-12">
            <div className="w-full md:flex-[2]">
              ...
              <h3 className="text-sm font-semibold tracking-wider uppercase mb-5" style={{ color: theme.text }}>{t('about.life')}</h3>
            ...
            <div className="w-full md:flex-1">
              ...
              <div className="mb-4">
                ...
                <p className="text-[10px] mt-4 text-center" style={{ color: theme.textSec, opacity: 0.4 }}>{t('about.mbti.footer')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-12">
            <div className="rounded-2xl cursor-pointer select-none transition-all duration-200 ease-out" onClick={toggleValues} style={{ backgroundColor: theme.bgDeep, border: `1px solid ${valuesHovered || valuesExpanded ? theme.border : theme.borderLight}`, boxShadow: valuesExpanded ? '0 2px 8px rgba(0,0,0,0.04)' : 'none' }} onMouseEnter={() => setValuesHovered(true)} onMouseLeave={() => setValuesHovered(false)}>
              <div className="flex items-center justify-between px-3 py-3 sm:px-5 sm:py-4">
                <span className="text-sm font-semibold tracking-wider uppercase" style={{ color: theme.text }}>{t('about.values')}</span>
          ...
          <div className="mt-12">
            <div className="rounded-2xl cursor-pointer select-none transition-all duration-200 ease-out" onClick={() => { if (!blindspotsExpanded) { setBlindspotsExpanded(true); setTimeout(() => setBlindspotsContentOpen(true), 100) } else { setBlindspotsContentOpen(false); setTimeout(() => setBlindspotsExpanded(false), 300) } }} style={{ backgroundColor: theme.bgDeep, border: `1px solid ${blindspotsHovered || blindspotsExpanded ? theme.border : theme.borderLight}`, boxShadow: blindspotsExpanded ? '0 2px 8px rgba(0,0,0,0.04)' : 'none' }} onMouseEnter={() => setBlindspotsHovered(true)} onMouseLeave={() => setBlindspotsHovered(false)}>
              <div className="flex items-center justify-between px-3 py-3 sm:px-5 sm:py-4">
                <span className="text-sm font-semibold tracking-wider uppercase" style={{ color: theme.text }}>{t('about.interpersonal')}</span>
              ...
              <div className="overflow-hidden" style={{ transition: 'max-height 0.7s ease-out', maxHeight: blindspotsContentOpen ? '1000px' : '0px' }}>
                <div className="grid grid-cols-1 gap-3 md:gap-4 px-3 pb-3 sm:px-5 sm:pb-5">
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 mb-1"><span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: '#5E8268' }} /><p className="text-xs tracking-wider uppercase" style={{ color: theme.text }}>{t('about.interpersonal.pros')}</p></div>
                    {pros.map((item) => (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 mb-1"><span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: '#A06060' }} /><p className="text-xs tracking-wider uppercase" style={{ color: theme.text }}>{t('about.interpersonal.cons')}</p></div>
                    {cons.map((item) => (
      ...
      {view === 'work' && (
        <div key="work" style={{ animation: 'fade-up 0.6s ease-out both', animationDelay: '150ms' }}>
          <div className="space-y-16">
            <div>
              <h3 className="text-sm font-semibold tracking-wider uppercase mb-5" style={{ color: theme.text }}>{t('about.bio')}</h3>
            ...
            <div>
              <h3 className="text-sm font-semibold tracking-wider uppercase mb-5" style={{ color: theme.text }}>{t('about.exp')}</h3>
              <div className="space-y-4">
                {[{ role: t('about.exp.role'), company: t('about.exp.company'), period: t('about.exp.period'), details: [t('about.exp.d1'), t('about.exp.d2'), t('about.exp.d3')] }].map((exp) => (
            ...
            <div>
              <h3 className="text-sm font-semibold tracking-wider uppercase mb-5" style={{ color: theme.text }}>{t('about.skills')}</h3>
